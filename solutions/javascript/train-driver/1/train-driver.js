...
export function fixListOfWagons(ids) {
  const [firstId, secondId, ...restOfIds] = ids;
  return [...restOfIds, firstId, secondId];
}
...
export function correctListOfWagons(ids, missingWagons) {
  const [locoPosition, ...idRest] = ids;
  return [locoPosition, ...missingWagons, ...idRest];
}
...
export function extendRouteInformation(information, additional) {
  const complete = {...information, ...additional};
  return complete;
}
...
export function separateTimeOfArrival(information) {
  const {timeOfArrival, ...cleanInfo} = information;
  return [timeOfArrival, cleanInfo];
}
